...
def tree(Train, Bias, point_data, data, grouped_feature, idx, pre_energy, threshold, params):
    ...
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            ...
            kernels = pca.components_
            p = (data[i,idx[i,:,j],:]-pca.mean_)@kernels[0,:].T
            q = (data[i,idx[i,:,j],:]-pca.mean_)@kernels[1,:].T
            r = (data[i,idx[i,:,j],:]-pca.mean_)@kernels[2,:].T
            ...
            local_data[i,j] = (data[i,idx[i,:,j],:]-pca.mean_)@kernels.T
            local_kernels[i,j] = kernels
            local_mean[i,j] = pca.mean_
