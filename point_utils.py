...
def salient_point_sample(pts, K):
    """
    Input:
        pts: pointcloud data, [B, N, C]
        K: number of samples
    ...
    knn.fit(pts)
    neighbors = knn.kneighbors(pts,return_distance=False)
    variance = np.zeros((1024,))
    for i in range(1024):
        new = pts[neighbors[i]]
        temp = pts[i].reshape(-1,1).T
        new = new - temp
    ...
    np.random.seed(0)
    # farthest = np.random.randint(0, N, (B,))
    farthest = np.argmin(variance, axis=-1)
    batch_indices = np.arange(B)
    for i in range(K):
        centroids[:, i] = farthest
        centroid = pts[batch_indices, farthest, :].reshape(B, 1, 3)
        ...
        distance[mask] = dist[mask]
        farthest = np.argmax(distance, axis=-1)
        neigh = neighbors[farthest]
        var_l = variance[neigh]
        index = np.argmax(var_l,axis=1)
        farthest = neigh[0,index]

    return index_points(pts, centroids), centroids[0]
...
def gather_fea(nn_idx, point_data, fea, local_kernels, local_mean):
    ...
    #         p = (pc_n_t[i,j,:,:]-pca.mean_)@kernels[0,:].T
    #         q = (pc_n_t[i,j,:,:]-pca.mean_)@kernels[1,:].T
    #         r = (pc_n_t[i,j,:,:]-pca.mean_)@kernels[2,:].T
    ...
    #         pc_n[i,j] = (pc_n_t[i,j,:,:]-pca.mean_)@kernels.T
